...
PRJ-009 Phase 1 的移动优先 PWA。它验证的不是“哪家餐厅最好”，而是用户能否先快速留下一餐，吃完后再复判排序，逐步建立自己的味觉人生档案。

## 当前闭环

`实时摄像头取景/快门 → 设备内 WebP 压缩 → 验证版 AI 起草 → 快速保存 → 吃完后复判 → 月榜/年榜/黑榜`

- 首次可载入 6 条明确标记的演示档案，或空白开始。
- 主入口通过 `getUserMedia` 调用实时摄像头，支持前后镜头切换和应用内快门；相册导入仅为摄像头不可用时的备用。
- 首页默认显示榜单；记录与排名完全分开，保存后进入待复判区，不强迫用户边吃边排。
- 月榜和年榜从同一套个人顺位实时派生，并可随时手动上移或下移。
- 私人黑榜用于记录踩雷与吐槽，可调顺位、移回待复判；界面明确它不是公开餐厅评分。
- 月榜、年榜和黑榜均可生成不含食物照片的黑白红排行榜 PNG；手机优先打开系统分享面板，否则下载文件。
...
- 用户验收：分别走一次“演示档案”和“空白开始”；完成摄像头记录、稍后复判、月/年榜调序、送进/移出黑榜、分享图片、刷新恢复、离线重载、匿名导出与清空。
