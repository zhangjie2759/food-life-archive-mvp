...
describe('anonymous validation export', () => {
  it('uses a strict event whitelist and excludes personal content keys', () => {
    const events: ValidationEvent[] = [
      { id: 'secret-stable-id', type: 'ranking_completed', timestamp: '2026-08-30T00:00:00.000Z', elapsedMs: 42_000, comparisonCount: 3 },
    ...
    expect(result.summary).toMatchObject({ completedRankings: 1, deferredRankings: 1, firstRecordMs: 42_000, medianComparisonCount: 3 })
  })

  it('exports only the first completed record time for cross-tester median analysis', () => {
    const events: ValidationEvent[] = [
      { id: 'later', type: 'ranking_completed', timestamp: '2026-08-30T00:02:00.000Z', elapsedMs: 18_000, comparisonCount: 2 },
      { id: 'first', type: 'ranking_completed', timestamp: '2026-08-30T00:01:00.000Z', elapsedMs: 55_000, comparisonCount: 4 },
    ]
